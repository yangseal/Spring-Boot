(function ($) {
	var path = window.CONTEXT_PATH + "/public/js/lib/treeTable/";
    ...
    $.fn.treeTable = function (opts) {
        ...
        function formatNode(tr) {
            ...
            //-------------下面一大段都是获取$preSpan---------
            if (cMap[id] == 0) {
                //如果是顶级节点，则没有prev_xxxxx
                var $preSpan = $('<span class="prev_xxxxx"></span>');
            } else {
                //先判断是否有上一个兄弟节点
                if (!$cur.attr('isFirstOne')) {
                    var $preSpan = $('#' + $cur.attr('prevId'), $treeTable).children("td").eq(opts.column).find('.prev_xxxxx').clone();
                } else {
                    var $parent = $('#' + cMap[id], $treeTable);
                    //没有上一个兄弟节点，则使用父节点的prev_xxxxx
                    var $preSpan = $parent.children("td").eq(opts.column).find('.prev_xxxxx').clone();
        ...
        //递归查找当前节点下的最后一个深层子节点
        $treeTable.getlastSidByPid = function(pid){
        	var sid = pMap[pid] && pMap[pid][pMap[pid].length - 1] || pid;
        	if(sid == pid){
        		return sid;
        	}else{
        	 	return $treeTable.getlastSidByPid(sid);
        	}
        };
        
        //添加行
        $treeTable.addRows = function(trsHtml){
        	var $trs = $(trsHtml);
            if (!$trs.length) { return false; }
            
            var pId = $($trs[0]).attr('pId');
            if (!pId) {
            	$treeTable.append($trs);
            }else{
            	//插入到最后一个孩子后面，或者直接插在父节点后面
            	var insertId = $treeTable.getlastSidByPid(pId);
                $('#' + insertId, $treeTable).after($trs);
            }
            initRelation($trs);
            open(pId);  
        	var pTR = $("#"+pId);
       		pTR.attr("hasChild","true");
       		if (pTR.find("td:first .vsStyle_active_node").length > 0)
           		pTR.find("td:first .vsStyle_active_node:last").attr("class","vsStyle_active_node vsStyle_last_open");            
       		else
       			pTR.find("td:first .vsStyle_node:last").attr("class","vsStyle_active_node vsStyle_last_open");            
        };        
        

        //删除指定行
		$treeTable.deleteRow = function(id){
			$.each($('tr.' + id, $treeTable), function(i,val){      
				$treeTable.deleteRow(val.id);
         	}); 
			$('tr.' + id, $treeTable).remove();

			$('tr#' + id, $treeTable).remove();
			var pid = cMap[id];
			if(pid == '0' || pid){
				var sIdsArray = pMap[pid];
				for(var index in sIdsArray){
					if(sIdsArray[index] == id){
						sIdsArray.splice(index,1);
						break;
					}
				}
				delete cMap[id];
